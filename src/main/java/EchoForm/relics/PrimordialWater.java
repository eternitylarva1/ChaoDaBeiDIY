...
public class PrimordialWater extends CustomRelic {
    ...
    public String getUpdatedDescription() {
        return DESCRIPTIONS[0] + (MAX_MIRACLES - this.miracleCount) + DESCRIPTIONS[1];
    }

    @Override
    public void onTrigger() {
        // 你可点击此遗物，然后获得一张奇迹（每场战斗限6次）
        if (this.miracleCount < MAX_MIRACLES && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) {
            this.flash();
            this.miracleCount++;
    ...
    public boolean checkTrigger() {
        // 检查是否可以触发
        return this.miracleCount < MAX_MIRACLES && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT;
    }
