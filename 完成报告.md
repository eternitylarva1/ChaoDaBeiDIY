...
14. **扭曲藤蔓** (TwistedVine) - 战斗开始时所有怪物最大生命降低25%
...
3. **扭曲藤蔓** - "所有？房间变为特殊的普通敌人战斗"：需要patch房间生成系统
4. **传送门** - "所有精英房间变为一个大颚虫"：需要patch房间生成系统
5. **催眠表** - "前两个回合开始时，你可以自由选择敌人的意图"：需要复杂的UI实现
...
- **Patch类**: 3个（DarkContractPatch、PlatePatch、PotionHelperPatch）
- **本地化文件**: 8个（ZHS和ENG的keywords、powers、relics、ui、potions）

## 总结

所有31个遗物已基本实现，其中大部分功能已完全可用。少数需要修改游戏核心逻辑的功能（如房间生成、遗物上限等）暂时保留简化实现，需要进一步研究游戏源码才能完成。
